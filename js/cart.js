...
function addToCart(id, name, price, image, clickEvent, size) {
  ...
  if (existing) {
    ...
    cart.push({ id: cartId, productId: id, name: name, price: price, image: image, quantity: 1, size: size || null });
  }
  ...
  miniCartItems.innerHTML = cart.map(item => `
    <div class="mini-cart-item">
      <div class="mini-cart-item-image">
        ${item.image ? `<img src="${item.image}" alt="${item.name.replace(/"/g, '&quot;')}" onerror="this.onerror=null;this.src='/images/placeholder-product.svg'">` : ''}
      </div>
  ...
  cartItems.innerHTML = cart.map(item => `
    <div class="cart-item">
      <div class="cart-item-image">
        ${item.image ? `<img src="${item.image}" alt="${item.name.replace(/"/g, '&quot;')}" onerror="this.onerror=null;this.src='/images/placeholder-product.svg'">` : '<span>No image</span>'}
      </div>
